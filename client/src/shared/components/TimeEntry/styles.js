...
export const ModalHeader = styled.div`
  background: linear-gradient(
    135deg,
    ${props => props.$gradientStart || color.violetGradientStart} 0%,
    ${props => props.$gradientEnd || color.violetGradientEnd} 100%
  );
  border-bottom: 1px solid ${props => props.$borderColor || color.violetBorderLight};
...
export const KeyBadge = styled.div`
  display: inline-flex;
  align-items: center;
  padding: 3px 10px;
  border-radius: ${radius.pill}px;
  background: ${props => props.$bg || color.statusViolet.bg};
  border: 1px solid ${props => props.$border || color.statusViolet.border};
  color: ${props => props.$text || color.statusViolet.text};
  ${font.medium}
